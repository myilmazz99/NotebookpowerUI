...
export const addEmail = (email) => async (dispatch) => {
  try {
    await webAPI.post("api/admin/email", email);
...
export const getEmails = () => async (dispatch) => {
  try {
    let response = await webAPI.get("api/admin/emails");
...
export const addFeedback = (feedback) => async (dispatch) => {
  try {
    await webAPI.post("api/admin/feedback", feedback);
    dispatchActionResult(
      dispatch,
      true,
...
export const getFeedbacks = () => async (dispatch) => {
  try {
    let response = await webAPI.get("api/admin/feedbacks");
    dispatch({
